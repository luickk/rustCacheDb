...
fn extended_client_test() {
    let mut cache_client = rustcachedb::CacheClient::<CacheString, CacheString>::create_connect([127, 0, 0, 1], 8080).unwrap();
    for _ in 1..500 {
        cache_client.push(rustcachedb::KeyValObj{key: CacheString(String::from("brian")), val: CacheString(String::from("test"))}).unwrap();
    }
}

#[test]
fn extended_server_test() { 
    let cache = rustcachedb::CacheDb::<CacheString, CacheString>::new([127, 0, 0, 1], 8080);
